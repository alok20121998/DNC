...
class Memory(nn.Module):
  ...
  def __init__(self, input_size, mem_size=512, cell_size=32, read_heads=4, gpu_id=-1, independent_linears=True):
    ...
    self.read_key_size = cell_size//8
    self.write_key_size = cell_size//4
    self.read_heads = read_heads
    self.gpu_id = gpu_id
    self.input_size = input_size
    self.independent_linears = independent_linears
    self.read_network = nn.Sequential(nn.Linear(self.mem_size*self.cell_size, self.cell_size*3))

    m = self.mem_size
    w = self.cell_size
    rk = self.read_key_size
    wk = self.write_key_size
    r = self.read_heads

    if self.independent_linears:
      self.read_keys_transform = nn.Linear(self.input_size, rk * r)
      self.read_strengths_transform = nn.Linear(self.input_size, r)
      self.write_key_transform = nn.Linear(self.input_size, wk)
      ...
      self.interface_size = (r * rk) + (5 * r) + wk + 3 + (2 * w)
  ...
  def write(self, write_key, write_vector, erase_vector, free_gates, read_strengths, write_strength, write_gate, allocation_gate, hidden):
    ...
    write_content_weights = self.content_weightings(hidden['memory'], write_key, write_strength, self.write_key_size)
  ...
  def read_vectors(self, memory, read_weights):
    scaled_memory = torch.einsum('bij,bjk->bijk', read_weights, memory)
    return self.read_network(torch.flatten(scaled_memory, start_dim=2))

  def read(self, read_keys, read_strengths, read_modes, hidden):
    content_weights = self.content_weightings(hidden['memory'], read_keys, read_strengths, self.read_key_size)
  ...
  def forward(self, ξ, hidden):
    ...
    if self.independent_linears:
      # r read keys (b * r * w)
      read_keys = T.tanh(self.read_keys_transform(ξ).view(b, r, k))
      # r read strengths (b * r)
      read_strengths = F.softplus(self.read_strengths_transform(ξ).view(b, r))
      # write key (b * 1 * w)
      write_key = T.tanh(self.write_key_transform(ξ).view(b, 1, k))
      # write strength (b * 1)
      ...
      # r read keys (b * w * r)
      read_keys = T.tanh(ξ[:, :r * rk].contiguous().view(b, r, rk))
      # r read strengths (b * r)
      read_strengths = F.softplus(ξ[:, r * rk:r * rk + r].contiguous().view(b, r))
      # write key (b * w * 1)
      write_key = T.tanh(ξ[:, r * rk + r:r * rk + r + wk].contiguous().view(b, 1, wk))
      # write strength (b * 1)
      write_strength = F.softplus(ξ[:, r * rk + r + wk].contiguous().view(b, 1))
      # erase vector (b * w)
      erase_vector = T.sigmoid(ξ[:, r * rk + r + wk + 1: r * rk + r + wk + 1 + w].contiguous().view(b, 1, w))
      # write vector (b * w)
      write_vector = T.tanh(ξ[:, r * rk + r + wk + 1 + w: r * rk + r + wk + 1 + 2 * w].contiguous().view(b, 1, w))
      # r free gates (b * r)
      free_gates = T.sigmoid(ξ[:,r * rk + r + wk + 1 + 2 * w: r * rk + 2 * r + wk + 1 + 2 * w].contiguous().view(b, r))
      # allocation gate (b * 1)
      allocation_gate = T.sigmoid(ξ[:, r * rk + 2 * r + wk + 1 + 2 * w].contiguous().unsqueeze(1).view(b, 1))
      # write gate (b * 1)
      write_gate = T.sigmoid(ξ[:, r * rk + 2 * r + wk + 2 + 2 * w].contiguous()).unsqueeze(1).view(b, 1)
      # read modes (b * 3*r)
      read_modes = σ(ξ[:, r * rk + 2 * r + wk + 3 + 2 * w: r * rk + 5 * r + wk + 3 + 2 * w].contiguous().view(b, r, 3), -1)
