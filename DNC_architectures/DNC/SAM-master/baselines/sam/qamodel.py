...
from baselines.sam.dnc import DNC
...
class QAmodel(nn.Module):
    def __init__(self, config: Dict[str, Any]):
        ...
        #                                 rel_size=96)
        self.update_module2 = DNC(
            config["symbol_size"], 
            hidden_size=179, 
            num_layers=config["num_layers"], 
            num_hidden_layers=config["num_hidden_layers"], 
            nr_cells=config["nr_cells"], 
    ...
    def forward(self, story: torch.Tensor, query: torch.Tensor):
        ...
        input_data = torch.cat((story_embed, torch.unsqueeze(query_embed, 1)), 1)
        logits, (chx, mhx, rv) = self.update_module2(input_data, (None, None, None), reset_experience=True, pass_through_memory=True)
